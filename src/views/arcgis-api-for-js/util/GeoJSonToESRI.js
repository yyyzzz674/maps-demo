 ...
 ************************************/
export function ConvertESRIToGeoJSON(esriObject) {
  ...
  function esriGeometryToGcGeometry(esriGeom) {
    var gcGeom, i, coordinates, geomType, geomParts, polyArray, ring

    //check for x, points, paths, or rings to determine geometry type.
    if (esriGeom) {
      //gcGeom = {};
      if (
        ((esriGeom.x && esriGeom.x !== 'NaN') || esriGeom.x === 0) &&
  ...
  return esriToGC(esriObject)
}

/************************************************
 * GeoJSON to ESRI Rest Converter
 ************************************************/
export function ConvertGeoJSONToESRI(geoJsonObject) {
  ...
  function isCompatible(esriGeomType, gcGeomType) {
    ...
    if (
      (esriGeomType === 'esriGeometryPoint' ||
        esriGeomType === 'esriGeometryMultipoint') &&
  ...
  function gcCoordinatesToEsriCoordinates(gcGeom) {
    ...
    if (gcGeom.type === 'MultiPoint' || gcGeom.type === 'MultiLineString') {
      ...
      if (gcGeom.coordinates) {
        esriCoords = gcPolygonCoordinatesToEsriPolygonCoordinates(
          gcGeom.coordinates
        )
      }
      ...
      if (gcGeom.coordinates) {
        for (i = 0, len = gcGeom.coordinates.length; i < len; i++) {
          esriCoords.push(
            gcPolygonCoordinatesToEsriPolygonCoordinates(gcGeom.coordinates[i])
          )
        }
      }
    }
  ...
  function gcGeometryToEsriGeometry(gcGeom) {
    ...
    //type to MultiPoint
    if (
      esriGeomInfo.type === 'esriGeometryPoint' &&
      gcGeometriesToConvert.length > 1
    ) {
  ...
  function gcToEsri(geoJsonObject) {
